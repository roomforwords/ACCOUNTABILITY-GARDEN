...
-- ==============================================================================
-- 3. COMMITMENTS TABLE
-- Every commitment gets a dedicated Banyan Tree.
-- No physical deletion. Only archive.
-- ==============================================================================
...
-- IMMUTABLE HISTORY
-- Once inserted, cannot be updated or deleted through normal user access.
-- ==============================================================================
...
CREATE TABLE IF NOT EXISTS public.daily_records (
  ...
  status TEXT NOT NULL CHECK (
    status IN ('COMPLETED', 'MISSED', 'PARTIAL', 'FROZEN')
  ),
  ...
  energy_level INTEGER CHECK (
    energy_level >= 1 AND energy_level <= 5
  ),
  ...
  CONSTRAINT unique_habit_record_per_date
...
-- ==============================================================================
-- 6. WATERING EVENTS
-- Permanent historical ledger of Banyan watering.
-- Append-only.
-- ==============================================================================
...
CREATE TABLE IF NOT EXISTS public.watering_events (
  ...
  CONSTRAINT unique_watering_event
...
-- ==============================================================================
-- 7. STREAK BREAKS
-- Permanent cumulative streak-break history.
-- NEVER UPDATE.
-- NEVER DELETE.
-- ==============================================================================
...
CREATE TABLE IF NOT EXISTS public.streak_breaks (
  ...
  CONSTRAINT unique_streak_break
    UNIQUE (user_id, habit_id, break_date)
);


-- ==============================================================================
-- 8. DAILY JOURNALS
-- Locked permanently upon save.
-- ==============================================================================
...
CREATE TABLE IF NOT EXISTS public.daily_journals (
  ...
  energy_level INTEGER NOT NULL DEFAULT 3 CHECK (
    energy_level >= 1 AND energy_level <= 5
  ),
  ...
  CONSTRAINT unique_user_journal_date
    UNIQUE (user_id, date)
);
...
CREATE INDEX IF NOT EXISTS idx_daily_records_user_id
  ON public.daily_records(user_id);

CREATE INDEX IF NOT EXISTS idx_daily_records_habit_date
  ON public.daily_records(habit_id, record_date);

CREATE INDEX IF NOT EXISTS idx_todos_user_id
  ON public.todos(user_id);

CREATE INDEX IF NOT EXISTS idx_watering_user_habit
  ON public.watering_events(user_id, habit_id);

CREATE INDEX IF NOT EXISTS idx_streak_breaks_user_habit
  ON public.streak_breaks(user_id, habit_id);
...
-- ==============================================================================
-- 11.2 COMMITMENTS POLICIES
-- No DELETE policy.
-- Commitments can only be archived through application logic.
-- ==============================================================================
...
-- ==============================================================================
-- END OF ACCOUNTABILITY GARDEN SCHEMA
-- ==============================================================================
